...
function insertShift(userId, date) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            VALUES (?, ?, ?);
        `;
            const shiftResult = yield new Promise((resolve, reject) => {
                db_1.pool.query(shiftSql, [userId, mysqlShiftStart, mysqlShiftEnd], (err, result) => {
                    if (err) {
        ...
        `;
            yield new Promise((resolve, reject) => {
                db_1.pool.query(breakSql, [shiftResult.insertId, mysqlBreakStart, mysqlBreakEnd], (err, result) => {
                    if (err) {
...
function initializeNovemberShifts() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield new Promise((resolve, reject) => {
                ...
                WHERE MONTH(s.shiftStart) = ?
                  ...
                  AND s.user_id = ?
            `;
                db_1.pool.query(clearSql, [month + 1, year, userId], (err, result) => {
                    if (err) {
            ...
            for (let day = 1; day <= lastDay; day++) {
                ...
                yield insertShift(userId, currentDate);
            }
